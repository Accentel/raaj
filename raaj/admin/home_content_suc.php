	...
	$tmp = $_FILES['image']['tmp_name'];
	 $dir = "../upload/home_gallery";
		       $destination =  $dir . '/' . $iname;
		         move_uploaded_file($tmp, $destination);
	//move_uploaded_file($tmp,"../staff/" . $code.$_FILES["sfile"]["name"]);
echo	 $iname1 =$code. $_FILES['image']['name'];
	//$iname = $code.$_FILES["sfile"]["name"];
	//$iname1 = mysql_real_escape_string($iname1);
	}else{
	 $iname1 = $inamek;
	 ...
	 $iname1;
	$a="update  home_content set image='$iname1',desc1='$desc1' where id='$id'";
	$sq=mysqli_query($link,$a); 
	if($sq){
	print "<script>";
			print "alert('Sucessfully Updated');";
			print "self.location='home_content1.php';";
			print "</script>";
	} else {
		print "<script>";
			print "alert('Unable To Save');";
			print "self.location='home_content_edit.php?id=$id';";
			print "</script>";
