	...
	if($con_fee>=1){
	 $valid_days=$row2['vdays'];
	   $valid=date('Y-m-d', strtotime("+$valid_days days"));
		...
		$sq1=mysqli_query($link,"select * from register where registerno");
		$r1=mysqli_fetch_array($sql);
		$valid=$r1['validity'];
		
	}
 $a="select dname1,doct_type from doct_infor where id = '$did'";
...
date_default_timezone_set('Asia/Kolkata');
 ...
 `gender`, `address`, `phoneno`, `registerfee`, `remarks`, `pat_type`, `pay_type`, `aadar`, `ref_doc`, `ref_doc_mob`,
  ...
  ,`policy`,`chq_num`,`bank`,`chq_date`, `insutype_name`, `policy_no`, `pkg_amt`, `req_amt`, `req_no`, `apr_date`, `ins_type`,`token_num`,`auth_by`,bill_num,serv_name,hosp_fee,opt_type,s_no,camp_type) VALUES 
 ...
 '$policy','$chq_num','$bank','$chq_datse','$insutype_name','$policy_no','$pkg_amt','$req_amt','$req_no','$apr_date','$ins_type','$token1','$ser','$cnt','$serv_name','$ser_amt','$appoint_type','$sno','$camp_type')"; 
