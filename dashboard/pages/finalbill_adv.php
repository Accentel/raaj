...
include('../db/connection.php');
 $sql='select * from ip_pat_admit  where  DIS_STATUS="ADMITTED" '; $r=mysqli_query($link,$sql) or die(mysqli_error());
while ($row=mysqli_fetch_array($r)) {echo  "<option value='$row[PAT_REGNO]'/>";}echo '</datalist>';?>
</td>
...
<?php 
			 include('../db/connection.php');
			   ...
			   if(isset($_REQUEST['submit'])){
			    ...
			    if($n != "")
				{
					...
					select  distinct a.PAT_REGNO,b.patientname from ip_pat_admit a,patientregister b  where a.PAT_REGNO =b.registerno and a.DIS_STATUS='ADMITTED' and a.PAT_REGNO='$n' order by a.PAT_REGNO desc");
				}
			   if($sq)
			   {
             ...
             <tr height="25px"><td style="text-align:center;"><input type="radio" name="empid" value="<?php echo $r ?>" onclick="javascript:showDoc(this.value);"/></td><td><?php echo $r ?></td><td><?php echo $h;?></td></tr>
				 ...
				 // $x="select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b
				  ...
				  $x="select distinct a.PAT_REGNO,b.patientname from ip_pat_admit a,patientregister b  where a.PAT_REGNO =b.registerno and a.DIS_STATUS='ADMITTED' order by a.PAT_REGNO desc";
