...
$l=0;
$sno=0;
$sql = mysqli_query($link,"select max(LR_NO+0) from phr_purinv_support");
if($sql)
{
...
$mrp_tab=$_REQUEST['mrp_tab'.$i];
		...
		//bal_qty=bal_qty+;
		$dis=$_REQUEST['disc'];
 $ss="select PRODUCT_NAME,tqty,BATCH_NO from phr_purinv_support1 where PRODUCT_NAME='$pname' and BATCH_NO='$batch'";
...
mysqli_query($link,"insert into stock(PRODUCT_NAME,BATCH_NO,T_QTY) values('$pname','$batch','$tp')");
		 ...
		 $as="insert into phr_purinv_support1(lr_no,product_code,product_name,packing_type,batch_no,mrp,exp_date,s_qty,s_free,s_rate,discount,value,vat,vat_amt,currentdate,auth_by,mfg_date,noitems,cost,balance_qty,manu_by,sgst,cgst,tqty,each_pur_rate,each_mrp_rate)
...
mysqli_query($link,"update  stock set T_QTY='$tq' where PRODUCT_NAME='$pname' and BATCH_NO='$batch'");
		$sql1=mysqli_query($link,"insert into phr_purinv_support1(lr_no,product_code,product_name,packing_type,batch_no,mrp,exp_date,s_qty,s_free,s_rate,discount,value,vat,vat_amt,currentdate,auth_by,mfg_date,noitems,cost,balance_qty,manu_by,tqty) 
