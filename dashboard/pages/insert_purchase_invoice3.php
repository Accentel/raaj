...
$qry=mysqli_query($link,"insert into phr_purinv_mast1 values($sno,'$supcode','$ptype','$invno','$invdt',now(),'$admin','$recby','$total','$recdt','$grn','$bankname','$chequeno','$date3','$paid','$bal','0')");
 ...
 $a="update phr_purinv_mast1 set total='$nettot',rec_by='$ses',SUPPL_CODE='$supcode',PUR_TYPE='$ptype',SUPPL_INV_NO='$invno',
...
//$dscamt=$_REQUEST['dscamt'][$i];
		...
		//bal_qty=bal_qty+;
		$dis=$_REQUEST['disc'];
$q=mysqli_query($link,"select * from phr_purinv_dtl1 where lr_no='$lrno'");
$p=mysqli_fetch_array($q);
$inv_id=$p['inv_id'];
$lrno1=$p['lr_no'];
if($_REQUEST['pname'][$i]!=""){
if(($inv_id=$invid)){
 $pp="update  phr_purinv_dtl1 set   product_name='$pname',batch_no='$batch',mrp='$mrp',exp_date='$expdt',s_qty='$sqty',s_free='$sfree',s_rate='$srate',
...
$sql1=mysqli_query($link,$pp);
 ...
 $mk="insert into phr_purinv_dtl1(lr_no,product_name,batch_no,mrp,exp_date,s_qty,s_free,s_rate,value,vat,vat_amt,currentdate,auth_by,mfg_date,noitems,balance_qty,manu_by,sgst,cgst,tqty,each_pur_rate,each_mrp_rate)
