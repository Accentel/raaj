...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	$dxx1=date($y.'-01-01');
$d2=date($y.'-12-31');
$aa="select count(1) as ct from bill where bdate = '$bdate' ";
...
$bno="BL-".$d.$m.$y."-".$reg;											
	...
	if($mrno != '') {
		$ts="INSERT INTO bill(mrno,billno,bdate,pname,age,gender,dname,mobile,ptype,time,tamount,discount,namount,pamount,bamount,remarks,user,paymenttype)VALUES
	...
	$bno = $crud->my_simple_crypt($bno,'e');
		...
		if($result1){
			...
			print "self.location='print_labbill1.php?id=$bno';";
			print "</script>";
		}
	}	
