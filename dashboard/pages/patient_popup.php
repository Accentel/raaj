...
<?php
	...
	{
		$n = $_REQUEST['name'];
		$sql = mysqli_query($link,"select distinct registerno,patientname from patientregister WHERE  patientname='$n' and status='' and pat_type!='Lab'");
		$rows = mysqli_num_rows($sql);
	}
	else
	{
		$sql = mysqli_query($link,"select distinct registerno,patientname from patientregister WHERE   status='' and pat_type!='Lab' order by reg_id desc");
		$rows = mysqli_num_rows($sql);
	}
?>
...
<!-------------------------------------------->
<tr align="center">
<table width="450px" class="sortable"  id="TABLE1" align="center">
  <thead>
    <tr>
