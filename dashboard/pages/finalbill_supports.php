...
function stateChanged(){ 
	...
	//alert('<%=current_date%>')
			...
			opener.document.getElementById("pname").value=strar[2];
			opener.document.getElementById("age").value=strar[3];
			opener.document.getElementById("gender").value=strar[4];
			opener.document.getElementById("dname").value=strar[5];
			opener.document.getElementById("mobile").value=strar[6];
			opener.document.getElementById("ptype").value=strar[7];
...
include('../db/connection.php');
 $sql='select distinct registerno from patientregister  '; $r=mysqli_query($link,$sql) or die(mysqli_error());
...
<?php 
			 include('../db/connection.php');
			   ...
			   if(isset($_REQUEST['submit'])){
			    ...
			    if($n != "")
				{
					...
					select  * from patientregister where registerno='$n' ");
				}
			   if($sq)
			   {
             ...
             <tr height="25px"><td style="text-align:center;"><input type="radio" name="empid" value="<?php echo $r ?>" onclick="javascript:showDoc(this.value);"/></td><td><?php echo $r ?></td><td><?php echo $h;?></td></tr>
				 ...
				 // $x="select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b
				  ...
				  $x="select distinct registerno,patientname from patientregister     order by registerno desc";
