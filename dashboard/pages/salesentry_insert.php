...
<a href="salesentry_list.php" class="button">Go to Sales Entry List</a>
...
$cnt=date('dmy')."-".$cnt1;
 ...
 values($lrno,'$btype','$custname','$lrno','$stype','$saledate','$total',now(),'$admin' ,'$bill','$custm_type',$card,'$age','$sex','$consultantname','$disc','$concessiontype','','','$mrnum','$spl_dis','$amount1','$cnt','$own','$own_amnt','') ";
...
$q1=mysqli_query($link,$x);
  ...
  $x="insert into phr_salent_mast
 values($lrno,'$btype','$custname','$lrno','$stype','$saledate','$total',now(),'$admin' ,'$bill','$custm_type',$card,'$age','$sex','$consultantname','$disc','$concessiontype','','','$mrnum','$spl_dis','$amount1','','$own','$own_amnt','') ";
...
$sq1=mysqli_query($link,"select * from phr_purinv_dtl where LR_NO='$invno'");
...
$xx="insert into phr_salent_dtl1(LR_NO, PRODUCT_NAME, U_QTY, U_RATE, VALUE, CURRENT, AUTH_BY, inv_id,disc,total,`gst`,gst_amt,batch_no) 
values($lrno,'$pname','$uqty','$urate','$value',now(),'$admin','$invno','$dis','$amt','$gst','$gst_amt','$batch1')";
...
if($due){ 
	...
	if($q2){
		...
		print "self.location='medbill.php?lr_no=$lrno';";
		print "</script>";
	}
}
	...
	print "<script>";
	print "alert('Successfully added');";
	print "self.location='medbill.php?lr_id=$lrno';";
	print "</script>";
}
