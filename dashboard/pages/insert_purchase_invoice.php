...
if($sql)
{
	...
	$sno=$sno+1;
 $mk="insert into phr_purinv_mast values($sno,'$supcode','$ptype','$invno','$invdt',now(),'$admin','$recby','$total','$recdt','$grn','$bankname','$chequeno','$date3','$paid','$bal','0')";

$qry=mysqli_query($link,$mk) or die(mysqli_error($link));
if($qry)
{
...
mysqli_query($link,"insert into stock(PRODUCT_NAME,BATCH_NO,T_QTY) values('$pname','$batch','$tp')") or die(mysqli_error($link));
		
		echo $as="insert into phr_purinv_dtl(lr_no,product_code,product_name,packing_type,batch_no,mrp,exp_date,s_qty,s_free,s_rate,discount,value,vat,vat_amt,currentdate,auth_by,mfg_date,noitems,cost,balance_qty,manu_by,sgst,cgst,tqty,each_pur_rate,each_mrp_rate)
...
echo $kkj="insert into phr_purinv_dtl(lr_no,product_code,product_name,packing_type,batch_no,mrp,exp_date,s_qty,s_free,s_rate,discount,value,vat,vat_amt,currentdate,auth_by,mfg_date,noitems,cost,balance_qty,manu_by,tqty,each_pur_rate,each_mrp_rate) 
		...
		$sql1=mysqli_query($link,$kkj) or die(mysqli_error($link));
}
