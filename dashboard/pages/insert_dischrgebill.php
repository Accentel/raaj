...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	if($rupees>=1){
		$p="insert into dis_bill(PAT_NO, ADV_DATE, ADV_AMT, PAYMENT_MODE, CURRENTDATE, AUTH_BY,bill_num,admit_date,pname,age,gender,mobile,rwords,mrno)
...
$sql1 = $crud->execute($p);
	...
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Discharge Collection','$rupees','$cnt','$mrno','$user','$paymenttype','$ndate','Advance Collection')";
	...
	$sql = $crud->execute($aa);
		...
		if($sql){
			...
			print "self.location='dis_billlist.php';";
			print "</script>";
		}
	}	
