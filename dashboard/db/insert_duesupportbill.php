...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Supports Due Bill','$damount','$cnt','$mrno','$user','$paymenttype','$ndate','Supports Due Bill')";
	...
	if($mrno != '') {
		  $ts="update supportsbill set mrno='$mrno',billno='$bno',bdate='$bdate',pname='$pname',age='$age',gender='$gender',dname='$dname',mobile='$mobile',ptype='$ptype',time='$time',tamount='$tamount',discount='$discount',namount='$namount',pamount='$paidamount',bamount='$ramount',remarks='$remarks',user='$user',paymenttype='$paymenttype' where id='$id'";
	...
	/*	for($t=0; $t<=count($_POST['sname']); $t++){
		...
		$amount= $_POST['amount'][$t];
		 $id1= $_POST['id1'][$t];
			if($tname!='' and $id1!=''){
			echo	  $ts1="update supportsbill1 set sname='$tname',amount='$amount',size='$size' where id='$id1' and mrno='$mrno'";
				$result1 = $crud->execute($ts1);
			}else{
				if($tname!='' and $id1==''){
			  echo	$ts2="insert into supportsbill1(bno,mrno,sname,amount,cdate,size)values('$bno','$mrno','$tname','$amount','$bdate','$size')";
				$crud->execute($ts2);
				}
			}
