...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	if($rupees>=1){
		$p="insert into adv_collection(PAT_NO, ADV_DATE, ADV_AMT, PAYMENT_MODE, CURRENTDATE, AUTH_BY,bill_num,admit_date,pname,age,gender,mobile,rwords,mrno,status)
 values('$patno','$bdate','$rupees','$paymenttype',now(),'$user','$cnt','$admitdate','$pname','$age','$gender','$mobile','$adv_words','$mrno','Dischage')";
$sql1 = $crud->execute($p);
} 
	
	
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Dischage Collection','$rupees','$cnt','$mrno','$user','$paymenttype','$ndate','Dischage Collection')";
